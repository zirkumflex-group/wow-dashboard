#!/usr/bin/env bash
set -euo pipefail

APP_DIR="${APP_DIR:-$HOME/wow-dashboard}"
ENV_FILE="${ENV_FILE:-}"
COMPOSE_FILE="${COMPOSE_FILE:-deploy/docker-compose.prod.yml}"
BACKUP_DIR="${BACKUP_DIR:-$HOME/wow-dashboard-backups/postgres}"
RETENTION_DAYS="${RETENTION_DAYS:-14}"

cd "$APP_DIR"

if [[ -z "$ENV_FILE" ]]; then
  if [[ -f deploy/.env.production ]]; then
    ENV_FILE="deploy/.env.production"
  elif [[ -f deploy/.env.staging ]]; then
    ENV_FILE="deploy/.env.staging"
    echo "Using legacy deploy/.env.staging. Rename it to deploy/.env.production after the production cutover."
  else
    ENV_FILE="deploy/.env.production"
  fi
fi

if [[ ! -f "$ENV_FILE" ]]; then
  echo "Missing env file: $ENV_FILE" >&2
  exit 1
fi

if [[ ! -f "$COMPOSE_FILE" ]]; then
  echo "Missing compose file: $COMPOSE_FILE" >&2
  exit 1
fi

compose_dir="$(dirname "$COMPOSE_FILE")"
env_dir="$(dirname "$ENV_FILE")"
if [[ -z "${SERVICE_ENV_FILE:-}" ]]; then
  if [[ "$env_dir" == "$compose_dir" ]]; then
    SERVICE_ENV_FILE="$(basename "$ENV_FILE")"
  else
    SERVICE_ENV_FILE="$ENV_FILE"
  fi
fi
export SERVICE_ENV_FILE

mkdir -p "$BACKUP_DIR"

timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup_name="wowdash-${timestamp}.dump"
backup_file="${BACKUP_DIR}/${backup_name}"
tmp_file="${backup_file}.tmp"

compose=(docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE")

echo "Writing Postgres backup to $backup_file"
"${compose[@]}" exec -T postgres \
  pg_dump -U wowdash -d wowdash --format=custom --no-owner --no-acl > "$tmp_file"

mv "$tmp_file" "$backup_file"
(
  cd "$BACKUP_DIR"
  sha256sum "$backup_name" > "${backup_name}.sha256"
  ln -sfn "$backup_name" latest.dump
  ln -sfn "${backup_name}.sha256" latest.dump.sha256
)

if [[ -n "${REMOTE_BACKUP_TARGET:-}" ]]; then
  echo "Copying backup to $REMOTE_BACKUP_TARGET"
  rsync -az "${backup_file}" "${backup_file}.sha256" "$REMOTE_BACKUP_TARGET"
fi

find "$BACKUP_DIR" -type f \
  \( -name "wowdash-*.dump" -o -name "wowdash-*.dump.sha256" \) \
  -mtime +"$RETENTION_DAYS" \
  -delete

echo "Backup complete: $backup_file"
