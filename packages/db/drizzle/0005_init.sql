CREATE TEMP TABLE "_character_normalized_merge_map" AS
WITH "ranked_characters" AS (
  SELECT
    "id" AS "source_id",
    first_value("id") OVER (
      PARTITION BY "player_id", "region", lower("realm"), lower("name")
      ORDER BY
    ...
    count(*) OVER (PARTITION BY "player_id", "region", lower("realm"), lower("name")) AS "group_size"
  FROM "characters"
)
...
SELECT
  'db.migration.merge_duplicate_normalized_character',
  jsonb_build_object(
    'sourceCharacter', to_jsonb("source_character"),
    'survivorCharacterId', "survivor_character"."id",
    'matchedBy', 'player_region_lower_realm_lower_name'
  ),
  now()
FROM "_character_normalized_merge_map" "merge_map"
...
--> statement-breakpoint
CREATE TEMP TABLE "_normalized_snapshot_rekey_duplicates" AS
WITH "rekeyed_snapshots" AS (
  ...
  FROM "snapshots"
  LEFT JOIN "_character_normalized_merge_map" "merge_map" ON "merge_map"."source_id" = "snapshots"."character_id"
  WHERE
    "snapshots"."character_id" IN (SELECT "source_id" FROM "_character_normalized_merge_map")
    OR "snapshots"."character_id" IN (SELECT "survivor_id" FROM "_character_normalized_merge_map")
),
"ranked_snapshots" AS (
  SELECT
    "id",
...
INSERT INTO "audit_log" ("event", "metadata", "timestamp")
SELECT
  'db.migration.merge_duplicate_normalized_snapshot',
  jsonb_build_object(
    'sourceSnapshot', to_jsonb("source_snapshot"),
    'survivorSnapshotId', "duplicates"."winner_id"
  ),
  now()
FROM "_normalized_snapshot_rekey_duplicates" "duplicates"
INNER JOIN "snapshots" "source_snapshot" ON "source_snapshot"."id" = "duplicates"."loser_id";
--> statement-breakpoint
DELETE FROM "snapshots"
WHERE "id" IN (SELECT "loser_id" FROM "_normalized_snapshot_rekey_duplicates");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TEMP TABLE "_normalized_daily_snapshot_rekey_duplicates" AS
WITH "rekeyed_daily_snapshots" AS (
  ...
  LEFT JOIN "_character_normalized_merge_map" "merge_map" ON "merge_map"."source_id" = "daily_snapshots"."character_id"
  WHERE
    "daily_snapshots"."character_id" IN (SELECT "source_id" FROM "_character_normalized_merge_map")
    OR "daily_snapshots"."character_id" IN (SELECT "survivor_id" FROM "_character_normalized_merge_map")
),
...
INSERT INTO "audit_log" ("event", "metadata", "timestamp")
SELECT
  'db.migration.merge_duplicate_normalized_daily_snapshot',
  jsonb_build_object(
    'sourceDailySnapshot', to_jsonb("source_daily_snapshot"),
    'survivorDailySnapshotId', "duplicates"."winner_id"
  ),
  now()
FROM "_normalized_daily_snapshot_rekey_duplicates" "duplicates"
...
WHERE "id" IN (SELECT "loser_id" FROM "_normalized_daily_snapshot_rekey_duplicates");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TEMP TABLE "_normalized_mythic_plus_run_rekey_duplicate_candidates" AS
WITH "rekeyed_runs" AS (
  ...
  LEFT JOIN "_character_normalized_merge_map" "merge_map" ON "merge_map"."source_id" = "runs"."character_id"
  WHERE
    "runs"."character_id" IN (SELECT "source_id" FROM "_character_normalized_merge_map")
    OR "runs"."character_id" IN (SELECT "survivor_id" FROM "_character_normalized_merge_map")
),
...
WHERE "id" <> "winner_id";
--> statement-breakpoint
CREATE TEMP TABLE "_normalized_mythic_plus_run_rekey_duplicates" AS
SELECT DISTINCT ON ("loser_id")
  "loser_id",
  "winner_id",
  "identity_type"
FROM "_normalized_mythic_plus_run_rekey_duplicate_candidates"
ORDER BY "loser_id", "priority", "winner_id";
--> statement-breakpoint
INSERT INTO "audit_log" ("event", "metadata", "timestamp")
SELECT
  'db.migration.merge_duplicate_normalized_mythic_plus_run',
  jsonb_build_object(
  ...
  now()
FROM "_normalized_mythic_plus_run_rekey_duplicates" "duplicates"
INNER JOIN "mythic_plus_runs" "source_run" ON "source_run"."id" = "duplicates"."loser_id";
--> statement-breakpoint
DELETE FROM "mythic_plus_runs"
WHERE "id" IN (SELECT "loser_id" FROM "_normalized_mythic_plus_run_rekey_duplicates");
--> statement-breakpoint
...
WITH "affected_survivors" AS (
  SELECT DISTINCT "survivor_id" AS "character_id" FROM "_character_normalized_merge_map"
),
"snapshot_stats" AS (
  SELECT
...
WHERE "id" IN (SELECT "source_id" FROM "_character_normalized_merge_map");
--> statement-breakpoint
DROP INDEX "characters_player_id_region_realm_name_uidx";
--> statement-breakpoint
ALTER TABLE "characters" ADD COLUMN "normalized_name" text GENERATED ALWAYS AS (lower("name")) STORED;
--> statement-breakpoint
ALTER TABLE "characters" ADD COLUMN "normalized_realm" text GENERATED ALWAYS AS (lower("realm")) STORED;
--> statement-breakpoint
CREATE UNIQUE INDEX "characters_player_id_region_realm_name_uidx" ON "characters" USING btree ("player_id","region","normalized_realm","normalized_name");
