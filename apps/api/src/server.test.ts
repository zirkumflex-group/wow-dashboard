...
describe("Phase 5 API routes", { concurrency: false }, () => {
  ...
  it("collapses stale completed Mythic+ duplicates with overlapping normalized starts", async () => {
    const auth = await seedAuthenticatedUser();
    const playerId = await seedAddonIngestOwner(
      auth,
      [
    ...
    const scoredStart = 1_779_798_012;
    const scoredCompletedAt = 1_779_799_324;
    const duplicateCompletedAt = 1_779_799_880;
    const durationMs = 1_314_194;
    const duplicateStart = duplicateCompletedAt - Math.floor(durationMs / 1000 + 0.5);
    const sharedMembers = [
      {
        name: "Overlapdupe",
        realm: "Blackhand",
        ...
        characters: [
          {
            ...
            mythicPlusRuns: [
              {
                fingerprint: `aid|attempt|17|558|10|${duplicateStart}`,
                attemptId: `attempt|17|558|10|${duplicateStart}`,
                observedAt: duplicateCompletedAt - 199,
                seasonID: 17,
                mapChallengeModeID: 558,
                mapName: "Magisters' Terrace",
                level: 10,
                ...
                completedAt: duplicateCompletedAt,
                endedAt: duplicateCompletedAt,
                members: [
                  ...sharedMembers,
                  { name: "Wonderfull", realm: "Archimonde", classTag: "HUNTER", role: "dps" },
                ],
    ...
    const character = await db.query.characters.findFirst({
      where: and(eq(characters.playerId, playerId), eq(characters.name, "Overlapdupe")),
    });
    assert.ok(character);
    assert.equal(character.mythicPlusRunCount, 1);
    assert.equal(character.mythicPlusRecentRunsPreview?.[0]?.runScore, 333);
    ...
    assert.equal(storedRuns.length, 1);
    assert.equal(storedRuns[0]?.runScore, 333);
    assert.equal(Math.floor((storedRuns[0]?.startDate?.getTime() ?? 0) / 1000), scoredStart);
  });
