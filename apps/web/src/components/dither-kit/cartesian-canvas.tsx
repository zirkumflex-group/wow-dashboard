...
function startCartesianLoop({
  ...
  let entranceReported = !state.current.animate || reduce;
  let intensity = 0;
  let needsFill = true;
  let lastPaintSig = "";
  ...
  const syncBloom = (s: ChartContextValue) => {
    if (!bloomCtx) return;
  ...
  const draw = (now: number) => {
    ...
    if (s.revision !== lastRevision) {
      ...
      lastProg = -1;
      entranceReported = !s.animate || reduce;
    }
    if (!animStart) animStart = now;
    const shouldAnimate = s.animate && !reduce;
    const prog = shouldAnimate
    ...
    let moving = false;
    const surfaceEase = s.animate && !reduce ? 0.18 : 1;
    for (const key of s.configKeys) {
      ...
      for (let x = 0; x < cols; x++) {
        ...
        if (Math.abs(dt) > 0.01 || Math.abs(df) > 0.01) {
          cur.top[x] += dt * surfaceEase;
          cur.floor[x] += df * surfaceEase;
          moving = true;
        } else {
          cur.top[x] = t.top[x];
    ...
    const reveal = shouldAnimate ? easeInOutCubic(prog) : 1;
    ...
    if (moving || settling || (shouldAnimate && prog < 1)) {
      schedule();
    } else if (s.sparkles && !reduce) {
      schedule(100);
    }
  };
...
export function CartesianCanvas() {
  ...
  const stars = useMemo(() => {
    ...
    return out;
  }, [ctx.sparkles, configKeys, dataLength, cols]);
  ...
  useEffect(() => {
    ...
    wakeRef.current = loop?.wake ?? null;
    return () => {
      if (wakeRef.current === loop?.wake) wakeRef.current = null;
      loop?.stop();
    };
  }, [cols, rows, hasBloom, isVisible]);
    ...
    <>
      ...
      {hasBloom ? (
        <canvas
          ref={bloomRef}
          aria-hidden="true"
          className="pointer-events-none absolute duration-200 ease-out motion-safe:transition-opacity motion-reduce:transition-none"
